...
class TestFilters(UnittestBase):
    ...
    def test_eq(self):
        ...
        where = {'yearPublished': 1}
        response = self.client.get(f'/book?where={json.dumps(where)}')
        assert response.result == self.schema.dump(self.books[:2])

        where = {'title': 'The Kite Runner', 'yearPublished': 1}
        response = self.client.get(f'/book?where={json.dumps(where)}')
    ...
    def test_lt(self):
        ...
        where = {'rating': {'$lt': 5}, 'yearPublished': {'$lt': 3}}
        response = self.client.get(f'/book?where={json.dumps(where)}')
        assert response.result == self.schema.dump(self.books[1:2])
    ...
    def test_lte(self):
        ...
        where = {'rating': {'$lt': 4}, 'yearPublished': {'$lt': 3}}
        response = self.client.get(f'/book?where={json.dumps(where)}')
    ...
    def test_gt(self):
        ...
        where = {'rating': {'$gt': 2}, 'yearPublished': {'$gt': 1}}
        response = self.client.get(f'/book?where={json.dumps(where)}')
    ...
    def test_gte(self):
        ...
        where = {'rating': {'$gte': 3}, 'yearPublished': {'$gte': 2}}
        response = self.client.get(f'/book?where={json.dumps(where)}')
        assert response.result == self.schema.dump(self.books[2:3])
